...
#     pred = predictions.sort('prediction', ascending=False).groupBy('user')
    ...
    pred = spark.sql("SELECT user, max(track) as track, max(prediction) as prediction, max(count) as count, COUNT(*) as num FROM df ORDER BY count [GROUP BY] user HAVING num <= 500")
