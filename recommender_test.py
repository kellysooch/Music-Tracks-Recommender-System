...
def main(spark, user_indexer_model, item_indexer_model, model_file, test_file):
    ...
    print("read file")
    test = test.sample(withReplacement = False, fraction = 0.0001)
    print("sample file")
...
#     predictions = model.predictAll(testData).rdd.map(lambda r: ((r.user, r.item), r.rating))
    predictionAndLabels = test_transformed.select(["user","item","prediction", "count"]).rdd.map(lambda r: (r.user, [r[3]])).reduceByKey(lambda p, q: p+q)
    print("made tuple")
